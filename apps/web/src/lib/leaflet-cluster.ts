...
// We keep exactly one mutable copy of `L` for the whole browser session and
// only import the plugin once against it — every caller (across every map
// component, and across client-side navigations within the same session)
// must reuse this same object, since the plugin's augmentation only ever
// lands on whichever object was `window.L` the first time it was imported.
