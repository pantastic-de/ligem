...
export type TermineSearchParams = {
  art?: string | string[];
...
// Builds a /termine URL from the current search params, with `overrides`
// applied on top (a value of undefined removes that key). `overrides.slug`
// is special-cased: when set, it becomes the /event/<slug> path (the
// selected event) instead of a query key; when explicitly cleared
// (undefined), the path falls back to the plain /termine list. Mirrors
// /projekte/projekte-page-view.tsx's buildProjekteHref.
function buildTermineHref(
  ...
  const path = slug ? `/event/${slug}` : "/termine";
...
/**
 * The shared two-column /termine layout (Zeitraum/Art/Zielgruppe filters +
 * map on the left, either the results list or one event's detail pane on
 * the right) — rendered both by the bare /termine list route and by
 * /event/[slug] (which resolves a slug to an event and passes it as
 * selectedEventId), so an event's own permalink always shows the same
 * search sidebar instead of the bare, sidebar-less view a standalone detail
 * page used to be. Mirrors /projekte/projekte-page-view.tsx's
 * ProjektePageView.
 */
export async function TerminePageView({
  searchParams: params,
  selectedEventId,
}: {
  ...
  const eventMapItems = events
    ...
    .map((e) => ({
      ...
      href: buildTermineHref(params, { slug: e.slug, angemeldet: undefined }),
    }));

  // Selecting an event (via its own /event/<slug> permalink, resolved by
  // the caller into selectedEventId) loads its detail inline in this same
  // right-hand column instead of a separate sidebar-less page — the
  // filters/calendar/map in the sidebar stay put either way. A stale/
  // invalid/unpublished id is treated as if no selection was made (falls
  // back to the results list) rather than erroring the whole page.
            ...
            <div className="rounded-2xl bg-surface p-4 sm:p-6 shadow-sm">
              <EventDetail
                ...
                prevItem={
                  prevEvent
                    ? {
                        href: buildTermineHref(params, { slug: prevEvent.slug, angemeldet: undefined }),
                ...
                nextItem={
                  nextEvent
                    ? {
                        href: buildTermineHref(params, { slug: nextEvent.slug, angemeldet: undefined }),
